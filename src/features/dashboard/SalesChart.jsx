...
function SalesChart({ bookings = [], numDays = 7 }) {
  ...
  if (numDays <= 0 || isNaN(numDays)) {
    console.error('Invalid numDays:', numDays);
    return <StyledSalesChart>Invalid date range</StyledSalesChart>;
  }
  ...
  try {
    ...
    // Validate dates
    if (!isValid(today) || !isValid(startDate)) {
    ...
    const data = allDates.map((date) => {
      const validBookings = bookings.filter((booking) => booking?.created_at && isValid(new Date(booking.created_at)));

      return {
        label: format(date, 'MMM dd'),
        totalSales: validBookings
          .filter((booking) => isSameDay(date, new Date(booking.created_at)))
          .reduce((acc, cur) => acc + (cur.totalPrice || 0), 0),
        extrasSales: validBookings
          .filter((booking) => isSameDay(date, new Date(booking.created_at)))
    ...
    return (
      <StyledSalesChart>
        <Heading type="h2">Error displaying sales data</Heading>
